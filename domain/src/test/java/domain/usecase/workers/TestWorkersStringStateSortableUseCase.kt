...
class TestWorkersStringStateSortableUseCase {
    ...
    lateinit var sortableUseCase: StringStateSortableUseCase<WorkersCloudSortableStateEntity,
            WorkerNameSortableEntity, WorkersStateEntity>

    private fun CoroutineScope.setupUseCase() = StringStateSortableUseCase(
        this.coroutineContext,
        TestFailMapper(),
        WorkersSortableStateToWorkersState(sortableWorkersToWorkersInfo)
    )
    ...
    fun `test with reversed list return sorted list`(): Unit = runBlocking {
        ...
        // c c, b b, a a
        val testData = WorkersCloudSortableStateEntity(testDataStore.testData)
    ...
    fun `test with empty list return sorted list`(): Unit = runBlocking {
        ...
        val testData = WorkersCloudSortableStateEntity(emptyList())
    ...
    fun `test with sorted list return list without changes`(): Unit = runBlocking {
        ...
        val testData = WorkersCloudSortableStateEntity(testDataStore.testData.reversed())
    ...
    class TestExceptionToStateMapper :
        Base.Mapper<WorkersCloudSortableStateEntity, WorkersStateEntity> {

        override fun map(model: WorkersCloudSortableStateEntity): WorkersStateEntity {
